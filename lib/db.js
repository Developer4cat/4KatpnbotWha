...
const { resolveMessageAuthor, resolveParticipantLabel, formatRankLine } = require("./participant");
const { evaluateRankAward, resetCounters } = require("./rank_antispam");
...
const levelUpThreshold = (level) => 5 * level ** 2 + 50 * level + 100;

const grantXp = async (sock, remoteJid, gid, uid, jid, sxp, notify = true) => {
	if (sxp <= 0) return null;

	if (await dbl.has(gid) && await dbl.has(`${gid}.${uid}`)) {
		let { xp, level } = await dbl.get(`${gid}.${uid}`);
		const lvlup = levelUpThreshold(level);
		if (parseInt(xp) + parseInt(sxp) >= lvlup) {
			const updated = await dbl.update(`${gid}.${uid}`, {
				$set: { xp: 0 },
				$inc: { level: 1 },
			});
			if (notify && sock && remoteJid) {
				await sock.sendMessage(remoteJid, {
					text: `Felicidades @${uid} has avanzado de nivel: ${updated.level} \n¡Sigue asi!\nUsa _${prefix}rank_ para ver la tabla de clasificacion.`,
					mentions: [jid],
				});
			}
			return updated;
		}
		await dbl.update(`${gid}.${uid}`, { $inc: { xp: parseInt(sxp) } });
		return await dbl.get(`${gid}.${uid}`);
	}

	await dbl.set(`${gid}.${uid}`, { xp: parseInt(sxp), level: 1 });
	return { xp: parseInt(sxp), level: 1 };
};
...
const evalLevel = async (sock, msg, content) => {
    try {
        ...
        const { uid, jid } = author;
        const award = evaluateRankAward(gid, uid, content);
        if (!award.allowed) return;

        await grantXp(sock, remoteJid, gid, uid, jid, award.xp, true);
    } catch (e) {
...
const getRank = async (gid, sock, remoteJid) => {
    ...
    if (has) {
        ...
        for (let i in keys) {
            ...
            if (val?.level == null || val?.xp == null) continue;
            dbObj[keys[i]] = val;
        }
        let dbArray = [];
        for (let x in dbObj) {
        ...
        const participants = remoteJid?.endsWith("@g.us")
            ? (await sock.groupMetadata(remoteJid)).participants
            : [];

        let i = 0;
        for (const entry of dbArray) {
            i += 1;
            if (i > 10) break;

            const { mentionJid } = await resolveParticipantLabel(
                sock,
                remoteJid,
                entry.id,
                participants,
            ...
            if (mentionJid) {
                result.text += formatRankLine(i, mentionJid, entry.level, entry.xp);
                result.mentions.push(mentionJid);
            } else {
                result.text += `${i}-. Contacto ~ Nivel: ${entry.level} ~ Experiencia: ${entry.xp}\n`;
            }
        }
    }
    return result;
...
const resetRankUser = async (gid, uid) => {
    if (await dbl.has(`${gid}.${uid}`)) {
        await dbl.delete(`${gid}.${uid}`);
    }
    resetCounters(gid, uid);
};

const resetRankGroup = async (gid) => {
    const keys = await dbl.keys(`${gid}`);
    for (const key of keys) {
        await dbl.delete(`${gid}.${key}`);
    }
    resetCounters(gid);
};

const resetRankAntiSpam = async (gid, uid = null) => resetCounters(gid, uid);

const adminGiveXp = async (sock, remoteJid, gid, uid, jid, amount) => {
    const xp = Math.max(0, parseInt(amount, 10) || 0);
    return grantXp(sock, remoteJid, gid, uid, jid, xp, true);
};

const adminSetLevel = async (gid, uid, level, xp = 0) => {
    const lvl = Math.max(1, parseInt(level, 10) || 1);
    const exp = Math.max(0, parseInt(xp, 10) || 0);
    await dbl.set(`${gid}.${uid}`, { level: lvl, xp: exp });
    return { level: lvl, xp: exp };
};

const adminAddLevel = async (gid, uid, amount) => {
    const inc = Math.max(0, parseInt(amount, 10) || 0);
    if (!(await dbl.has(`${gid}.${uid}`))) {
        return adminSetLevel(gid, uid, 1 + inc, 0);
    }
    const updated = await dbl.update(`${gid}.${uid}`, { $inc: { level: inc } });
    return updated;
};
