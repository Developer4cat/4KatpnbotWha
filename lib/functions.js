...
const appendLog = (message) => {
	...
	try {
		...
		console.error("No se pudo escribir en logs.txt:", e);
	}
};
...
const runFfmpeg = (args, timeoutMs = 90000) =>
	new Promise((resolve, reject) => {
		...
		proc.on("error", (err) => {
			clearTimeout(timer);
			if (err.code === "ENOENT") {
				reject(new Error("ffmpeg no está instalado o no está en PATH. Instálalo o define FFMPEG_PATH en .env"));
			} else {
				reject(err);
			}
		});
...
const normalizeStickerVideo = async (buffer, kind = "video") => {
	const suffix = crypto.randomBytes(4).toString("hex");
	const inputExt = kind === "gif" || isGifBuffer(buffer) ? "gif" : "mp4";
	const inputPath = path.join(os.tmpdir(), `4ktbot-sticker-in-${Date.now()}-${suffix}.${inputExt}`);
	const outputPath = path.join(os.tmpdir(), `4ktbot-sticker-out-${Date.now()}-${suffix}.webp`);
	try {
		...
		if (!fs.existsSync(outputPath)) {
			throw new Error("ffmpeg no generó el archivo webp");
		}
		return fs.readFileSync(outputPath);
	} catch (e) {
		appendLog(`normalizeStickerVideo: ${e.message || e}`);
		console.error("Error al normalizar video/gif para sticker:", e);
		throw e;
	} finally {
...
const prepareStickerMedia = async (buffer, kind) => {
	if (kind === "video" || kind === "gif") {
		return await normalizeStickerVideo(buffer, kind);
	}
	if (kind === "image" && isGifBuffer(buffer)) {
		return await normalizeStickerVideo(buffer, "gif");
	}
	if (kind === "image") {
...
const buildStickerBuffer = async (buffer, kind) => {
	const isAnimated = kind === "video" || kind === "gif" || (kind === "image" && isGifBuffer(buffer));
	if (isAnimated) {
		const mediaKind = kind === "gif" || (kind === "image" && isGifBuffer(buffer)) ? "gif" : "video";
		return await normalizeStickerVideo(buffer, mediaKind);
	}
	const prepared = await normalizeStickerImage(buffer);
	return await sticker(prepared);
};
