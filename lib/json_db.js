...
class JsonDB {
    ...
    delete(key) {
        ...
        if (!blockName) return false;
        const data = this._loadBlock(blockName);
        const { parent, key: lastKey } = this._getNode(data, parts, false);
        if (!parent || !Object.prototype.hasOwnProperty.call(parent, lastKey)) return false;
        delete parent[lastKey];
        if (parts.length === 1) {
            delete this._index.map[rootKey];
            const meta = this._getBlockMeta(blockName, false);
            if (meta) meta.roots = Math.max(0, Object.keys(data).length);
        }
        this._scheduleFlush(blockName);
        this._dirtyIndex = true;
        return true;
